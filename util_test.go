...
func TestCleanMessageHashtags(t *testing.T) {
	assert.Equal(t, cleanMessageHashtags("Hello"), "Hello", "Cleaning without hashtags and mentions should do nothing")

	// Hashtags
	assert.Equal(t, cleanMessageHashtags("Hello #topic"), "Hello", "Should clean hashtags")
	assert.Equal(t, cleanMessageHashtags("Hello #topic #topic-2"), "Hello", "Should clean all hashtags")
}
